...
train_image_path = Path("media/wikiart/images/train")
test_image_path = Path("media/wikiart/images/testval")
train_caption_path = "media/wikiart/annotations/train.json"
test_caption_path = "media/wikiart/annotations/testval.json"
vi_caption_path = "media/wikiart/annotations/annotations.json"
...
class CustomDataset(Dataset):
    ...
    def __init__(self, root_dir, captions_file, wikiart_file=vi_caption_path, transform=None, freq_threshold=1):
        self.root_dir = root_dir
        self.captions_file = captions_file
        self.data = json.load(open(captions_file, "r"))
        self.transform = transform
        self.imgid2imgname = {entry['id']: entry['file_name'] for entry in self.data['images']}
        self.captions = [ann['caption'] for ann in self.data['annotations']]

        all_captions = json.load(open(wikiart_file, "r"))
...
class DictDataset(CustomDataset):
    ...
    def __init__(self, root_dir, captions_file, wikiart_file=vi_caption_path, transform=None, freq_threshold=1):
        super().__init__(root_dir, captions_file, wikiart_file, transform, freq_threshold)
    ...
    def img_id_2_captions(self):
        ...
        for ann in self.data['annotations']:
            img_id_2_captions[ann['image_id']].append(" ".join(self.vocab.tokenize(ann['caption'])))
...
if __name__ == "__main__":
    ...
    test_captions = [ann['caption'] for ann in test_data['annotations']]
